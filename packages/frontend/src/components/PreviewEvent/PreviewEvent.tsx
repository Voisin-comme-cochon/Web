...
export default function PreviewEvent({ event, user }: { event: EventModel; user: UserModel }) {
    return (
        <div className="relative rounded-2xl max-w-80 w-full overflow-hidden shadow-lg bg-gray-100">
            <img src={event.photo} alt={event.name} className="w-full h-28 object-cover" />
            <div className="absolute bottom-0 left-0 w-full bg-white rounded-b-2xl flex flex-col gap-2 p-4 z-10">
                <div className="flex flex-row justify-between items-center">
                    ...
                    <p className="text-sm text-gray-600">Le {new Date(event.dateStart).toLocaleDateString()}</p>
                </div>
                <div className="flex flex-row justify-between items-center">
