...
export function useChatManager({ neighborhoodId, currentUserId }: UseChatManagerProps) {
    ...
    const handleMessageReceived = useCallback((message: GroupMessageModel) => {
        ...
        messaging.setGroups(prev => prev.map(group => 
    ...
    const selectConversation = useCallback((groupId: number | null) => {
        ...
        if (groupId) {
            ...
            if (!webSocket.connected) {
                ...
                setTimeout(() => {
                    console.log('ChatManager: Retry joinGroup après timeout, WebSocket connecté:', webSocket.connected);
                    if (webSocket.connected) {
    ...
    const sendMessage = useCallback(async (content: string, groupId: number): Promise<boolean> => {
    ...
    const joinAndSelectGroup = useCallback(async (groupId: number): Promise<boolean> => {
    ...
    const createAndSelectGroup = useCallback(async (groupData: any): Promise<GroupModel | null> => {
    ...
    const createAndSelectPrivateChat = useCallback(async (targetUserId: number): Promise<GroupModel | null> => {
